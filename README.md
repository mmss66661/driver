...
该功能使用左右轮累计编码器确定行程进度，使用 IMU601 Yaw 记录和闭环修正航向。
距离始终以编码器 tick 表示，因此第一版不依赖轮径、轮距和编码器线数，也不使用
容易快速漂移的加速度二次积分。
...
4. 向前推动车辆走完整条路线，再按 KEY1 停止并写入 Flash；
5. OLED 显示 `NAV:DONE`、`PATH:SAVED` 后，把车放回相同起点和相同初始朝向；
6. 再按 KEY1 开始自动回放；首个非零速度命令会让驱动板进入闭环；
7. 运行中按 MODE 或 KEY1 都会立即停止底盘；
8. 已有路径时，按住 UP 再按 KEY1 可覆盖并重新示教。

驱动板协议没有退出闭环命令。若 OLED 显示 `FAULT:POWER`，说明本次上电已经运行过
电机闭环；需要断电重启后，在按 MODE 启动车辆之前重新开始手推示教。

路径保存在主 Flash 末尾 `0x1E000..0x1FFFF` 的 8 KiB 区域，带版本、点数和 CRC32
校验，最多保存 1022 个路径点。普通断电不会丢失；重新烧录程序时调试器可能执行
整片擦除，因此烧录后需要重新示教。链接脚本用 NOLOAD 段占住该区域，固件变大并
与路径区冲突时会直接链接失败。

驱动板会主动发送 `0A 03 08 ... CRC`：4 个电机各占一个有符号 16 位累计编码器值，
高字节在前。电机1映射左轮，电机2映射右轮，电机3/4不参与当前底盘。导航按16位
模运算处理 `32767 -> -32768` 回绕，不会把它误判为约65535 tick的跳变。

回放默认速度、航向增益和最大转向量集中定义在 `path_navigation.c`。车辆转向方向
相反时修改 `PATH_STEERING_INVERTED`；实车首次测试应架空驱动轮并随时准备按 MODE。

路径回放同时读取12路巡线模块，并由导航模块生成唯一的底盘命令：

- 按编码器轮程对记录航向做线性插值，并预瞄前方60 tick的目标航向；
- 再查看前方180 tick的航向变化，提前识别弯道并从速度10降到6至7；
- 普通黑线段以惯导航向作为前馈，再叠加75%的巡线横向修正；
- 检测到6路及以上黑线时视为宽线或路口，只叠加25%的巡线修正，避免横线误导；
- 完全丢线时降到速度3，仅按记录航向寻找黑线；800 ms仍未找到则停车并显示
  `FAULT:LINE`；
- 航向误差超过35°时原地修正，转向指令每20 ms最多变化1，最大原地转向量为8。

示教时应沿黑线中心缓慢、连续地推车，直角处也要让左右轮真实滚动完成转向；不要
提起车辆旋转或强行拖滑轮胎。回放必须放回同一起点、保持相同初始朝向，并沿示教时
相同方向运行。预瞄距离、巡线叠加比例、丢线时间和各阶段速度集中由
`PATH_REPLAY_*`、`PATH_*_LINE_WEIGHT` 和 `PATH_LINE_LOST_TIMEOUT_MS` 调整。
