...
- `LineFollower_getObservation()`：只计算原始位图、黑线数量、中心误差、转向建议和
  普通线/路口/丢线状态，不直接控制电机，供融合导航使用。

`line_follower.c` 默认假定 LINE0 在车体左侧、LINE11 在右侧，每 20 ms 对黑线通道
做加权平均。单独巡线时默认直线速度为 `300`，弯道降为 `180`，宽线/路口降为
`140`，转向修正上限为 `220`；没有任何黑线输入时立即停车。若传感器排列或转向
方向相反，修改
`LINE_FOLLOW_STEERING_INVERTED`；速度和灵敏度分别调整 `LINE_FOLLOW_BASE_SPEED`、
`LINE_FOLLOW_TURN_GAIN` 和 `LINE_FOLLOW_MAX_TURN`。
...
- 12 路巡线已按加权中心输出固定基准速度和差速转向，无线时立即停车；
...
- 尚未填写轮径、轮距、减速比、编码器线数；示教导航使用原始 encoder tick，仍不能
  输出 m/s、米制里程和标准二维里程计；
- 巡线与惯导航向已经按可信状态融合并支持弯道降速、路口惯导优先和短时丢线恢复；
