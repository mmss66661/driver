...
- `ChassisMotor_init()`：配置电机1/2编码器方向，保持默认非闭环状态；
...
- `ChassisMotor_getStatus()`：读取命令、4 路16位累计编码器和通信错误计数。

驱动板主动上报的电机1编码器保存为 `leftEncoderCount`，电机2保存为
`rightEncoderCount`；两者均为符号扩展后的16位累计值。
